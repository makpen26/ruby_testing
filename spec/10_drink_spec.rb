...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:person_drink) { described_class.new('americano') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(person_drink.type).to eq('americano')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(person_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:person_drinks) { described_class.new('americano') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(person_drinks).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:person_drinkss) { described_class.new('americano', 15) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(person_drinkss).not_to be_full
      end
    end
  end
end
