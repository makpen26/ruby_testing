...
describe Dog do
  ...
  subject(:kai) { described_class.new(name: 'kai', breed: 'german', color: 'white') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog has method name share with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(kai).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(kai).to be_sleeping
  end
end
