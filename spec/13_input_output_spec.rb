...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      subject(:game_not_over) { described_class.new(5, '7') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        letters = 'b'
        valid_input = '1'
        allow(game_loop).to receive(:player_input).and_return(letter, letters, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      subject(:game_two) { described_class.new(1, '3', 3) }
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game_three) { described_class.new(9, '3', 10) }

      it 'outputs correct phrase' do
        else_phrases = "That was hard. It took you 10 guesses!\n"
        expect { game_three.final_message }.to output(else_phrases).to_stdout
      end
    end
  end
end
