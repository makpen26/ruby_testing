...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:object_one) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        allow(number_guessing).to receive(:value).and_return(8)
        result = object_one.make_guess
        expect(result).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:object_two) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        allow(number_guessing).to receive(:value).and_return(8)
        result = object_two.make_guess
        expect(result).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:object_three) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        allow(number_guessing).to receive(:value).and_return(8)
        result = object_three.make_guess
        expect(result).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:object_four) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        allow(number_guessing).to receive(:value).and_return(8)
        result = object_four.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_guess) { double('number_guess', value: 9) }
      subject(:game_overs) { described_class.new(5, 10, number_guess, 9) }
      ...
      it 'is game over' do
        expect(game_overs).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_end) { double('number_end', value: 23) }
      subject(:game_end) { described_class.new(19, 27, number_end, 24) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge_object) { described_class.new(7, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        edge_object.update_range
        minimum = edge_object.min
        expect(minimum).to eq(8)
      end

      it 'does not update max' do
        edge_object.update_range
        maximum = edge_object.max
        expect(maximum).to eq(8)
      end
    end
  end
