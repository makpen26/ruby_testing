...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid_input_as_letter = 'q'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(invalid_input_as_letter, valid_input)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_input_as_symbol = '@'
        another_invalid_input_as_letter = 'z'
        valid_input = '9'
        allow(game_input).to receive(:gets).and_return(invalid_input_as_symbol, another_invalid_input_as_letter, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    subject(:game_test1) { described_class.new(7, 15) }

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = game_test1.instance_variable_get(:@minimum)
        max = game_test1.instance_variable_get(:@maximum)
        user_valid_input = 8
        verified_input = game_test1.verify_input(min, max, user_valid_input)
        expect(verified_input).to eq(8)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_turn_order) { described_class.new(10, 20) }
    let(:turn_order_verify_double) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(game_turn_order).to receive(:display_guess)
      allow(turn_order_verify_double).to receive(:make_guess)
      allow(turn_order_verify_double).to receive(:update_range)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(turn_order_verify_double) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(turn_order_verify_double).to receive(:make_guess).once
      game_turn_order.display_turn_order(turn_order_verify_double)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(turn_order_verify_double).to receive(:update_range).once
      game_turn_order.display_turn_order(turn_order_verify_double)
    end
