...
describe Cat do
  ...
  subject(:sophy) { described_class.new('Sophy', 'British Shorthair') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when Cat has method name share with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(sophy).to_not be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(sophy).to be_hiding
  end
end
